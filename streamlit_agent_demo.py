"""Streamlit demo: Health Coach agent + Assignment 4 TRACE eval dashboard."""
...
def _ensure_demo_dataset() -> str:
    """Create the synthetic Marcus dataset once per process if it is missing."""
    user = ensure_demo_health_data()
    return user.display_name


_ensure_demo_dataset()

coach_tab, eval_tab = st.tabs(["Health Coach", "TRACE Evals"])


def _render_health_coach_demo() -> None:
    ...
    if st.button("Analyze Health", type="primary"):
        ...
        with st.expander("Debug — sanitized structured trace"):
            st.code(
                json.dumps(json.loads(result.trace_path.read_text(encoding="utf-8")), indent=2),
                language="json",
            )


with coach_tab:
    _render_health_coach_demo()

with eval_tab:
    render_trace_eval_dashboard()
